...
knitr::opts_chunk$set(echo = TRUE) #instead of echo :: in quarto
library(tidyverse)
```
...
# project population size change between life cycles year to year#
# ------ comment line-by-line ---- #

time_yr <- seq(from = 0, to = 8, by = 1) # time series from 0-8, so from year 0 to year 8 we are estimaing population size

# creating an empty matrix so we can have the same number of age transitions and the number of projections in the same matrix
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# filling the first year's data into the first row
proj_year[1, ] <- fish_year0

# taking the dot product from the start year and the leslie matrix, which translates to an estimation of the next year's population
for (i in 2:length(time_yr)) { #start with year 1 (in the 2nd position in the vector)
  proj_year[i,] <- fish_leslie %*% proj_year[i-1,]
}

# The rest is wrangling & visualization (run to check it out):
colnames(proj_year) <- c("eggs", "fry", "juvenile", "adult") # naming the columns from the matrix

#setting the dimensions of the data frame so we can plot it- specifying year, lifestage, and population as columns instead
proj_df <- data.frame(time_yr, proj_year) %>% 
  pivot_longer(cols = -time_yr, names_to = "lifestage", values_to = "stage_population") #converting to long form from wide
